...
#### Consul Enterprise using non-`default` partition and non-`default` namespace

You can specify an admin partition and namespace when using Consul Enterprise. The following example policy allows a terminating gateway to register as a service named `terminating-gateway` in the `default` partition and `ns1` namespace. 
...
consul acl policy create \
  -name tgw-register -rules @tgw-register.hcl \
  -description "terminating gateway policy" \
  -partition ptn1 -namespace default
```
